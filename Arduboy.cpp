// This Arduboy.cpp has some modified functions to accommodate two screen buffers.
// These will largely be of little use in any other project because of how much RAM
// this takes up.  The only reason I included two is for faster flickering back and
// forth between them, due to the amount of time the copyScreen function takes.
// Normally, bitmaps should be pre-converted to the Arduboy's screen format, not done
// within in the program.  This is simply a greyscale test so it's very quick and dirty.
...
{
  for (int a = 0; a < (HEIGHT*WIDTH)/8; a++) sBuffer0[a] = 0x00;
}
      ...
      for (int iCol = 0; iCol<w; iCol++)
      {
        ...
        if (iCol + x > 0)
        {
          if (bRow >= 0)
          {
            if (color)
            {
              this->sBuffer0[ (bRow*WIDTH) + x + iCol  ]
          ...
          if (yOffset && bRow<(HEIGHT/8)-1 && bRow > -2)
          {
            if (color)
            {
              this->sBuffer0[ ((bRow+1)*WIDTH) + x + iCol  ]
...
void Arduboy::display(uint8_t bufferNum)
{
  if (bufferNum == 0) {
    this->drawScreen(sBuffer0);
  }
  else {
    this->drawScreen(sBuffer1);
  }
}
...
// ekem gets most of the credit for this function.
// Originally it would convert a screen-formatted bitmap back to normal,
// I modified it to convert a normal bitmap to the screen format.

void Arduboy::copyScreen(const uint8_t *bitmap, uint8_t bufferNum)
{
  for (int j = 0; j < HEIGHT; j+=8)
        {
                for (int i = 0; i < WIDTH; i++)
                {
                    unsigned char c = pgm_read_byte(bitmap + ((j/8)*WIDTH+i));
                        ...
                        for (int k = 0; k < 8; k++)
                        {
                                ...
                                {
                                  if (bufferNum == 0) {
                                    sBuffer0[(j * 16) + ((i * 8) % WIDTH) + (7 - k)] |= 1 << ((i / 16) % 8);
                                  }
                                  else {
                                    sBuffer1[(j * 16) + ((i * 8) % WIDTH) + (7 - k)] |= 1 << ((i / 16) % 8);
                                  }
                                }
                        }
                }
